    ...
    >
      <div className="container mx-auto px-4">
        <div className="flex items-center justify-between h-20">
          ...
          <Link to="/">
            <motion.div
              className="text-2xl md:text-3xl font-orbitron font-bold cursor-pointer"
          ...
          <div className="hidden lg:flex items-center gap-8">
            ...
            >
              ...
              <Link to="/contact">
                <Button
                  variant="outline"
                  className="font-inter font-semibold border-border hover:bg-accent hover:text-accent-foreground"
                >
                  Contact
                </Button>
              </Link>
            ...
            {isAuthenticated ? (
              ...
              >
                <DropdownMenu>
                  ...
                  >
                    ...
                    {isAdmin && (
                      <Link to="/admin">
                        <DropdownMenuItem className="text-primary cursor-pointer hover:bg-accent focus:bg-accent focus:text-primary">
              ...
              >
                <Link to="/auth">
                  <Button className="bg-primary hover:bg-primary/90 text-primary-foreground font-inter font-semibold shadow-[0_0_15px_rgba(255,69,0,0.4)]">
                    Sign In
                  </Button>
                </Link>
              </motion.div>
          ...
          {isMobileMenuOpen && (
            <>
              ...
              <motion.div
                ...
                exit={{ opacity: 0 }}
                transition={{ duration: 0.2 }}
                className="lg:hidden fixed inset-0 top-20 bg-foreground/20 dark:bg-foreground/30 z-40"
              ...
              <motion.div
                ...
                className="lg:hidden absolute left-3 right-3 top-[calc(100%+0.5rem)] z-50 origin-top rounded-2xl border border-border bg-popover text-popover-foreground shadow-2xl overflow-hidden"
              >
                  ...
                  {navLinks.map((link) => {
                    ...
                    return (
                      <button
                        key={link.name}
                        onClick={() => handleNavClick(link.href, link.isExternal)}
                        className="w-full flex items-center gap-3 px-3 py-2.5 rounded-lg text-foreground hover:bg-accent hover:text-primary transition-colors font-inter font-medium text-sm group"
                      >
                        <Icon
                          size={18}
                ...
                {isAuthenticated && (
                  <div className="px-2 py-2 border-t border-border">
                    ...
                    <Link to="/profile-settings" onClick={() => setIsMobileMenuOpen(false)}>
                      <button className="w-full flex items-center gap-3 px-3 py-2.5 rounded-lg text-foreground hover:bg-accent hover:text-primary transition-colors text-sm font-medium">
                ...
                {/* Footer actions */}
                <div className="px-3 py-3 border-t border-border bg-muted/40 space-y-2">
                  <Link to="/contact" onClick={() => setIsMobileMenuOpen(false)}>
                    <Button
                      variant="outline"
                      className="w-full font-inter font-semibold gap-2 border-border text-foreground hover:bg-accent hover:text-accent-foreground"
                    >
                      <Mail size={16} />
                      Contact Us
                    </Button>
                  ...
                  {isAuthenticated ? (
                    ...
                    <Link to="/auth" onClick={() => setIsMobileMenuOpen(false)}>
                      <Button className="w-full bg-primary hover:bg-primary/90 text-primary-foreground font-inter font-semibold shadow-[0_0_15px_rgba(255,69,0,0.4)]">
                        Sign In
                      </Button>
                    </Link>
                  )}
