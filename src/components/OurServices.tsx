...
// ─── Asset paths ──────────────────────────────────────────────────────────────
const AUTO_DOSE_LOGO = "/src/assets/autodose.png";
...
const SPEED    = 0.28; // rad/s
const BASE_R   = 210;  // px — scales down on mobile (increased for taller 16:9 cards)
...
const NeuralNoiseBg = memo(() => {
  ...
  useEffect(() => {
    ...
    gl.attachShader(prog, mkShader(`
      ...
      varying vec2 vUv; attribute vec2 a;
      void main(){vUv=.5*(a+1.);gl_Position=vec4(a,0,1);}
    `, gl.VERTEX_SHADER));
    gl.attachShader(prog, mkShader(`
      precision mediump float;
      varying vec2 vUv;
      ...
      float ns(vec2 u,float t,float p){
        ...
        for(int j=0;j<15;j++){
          u=rot(u,1.);s=rot(s,1.);
          vec2 l=u*sc+float(j)+s-t;
          s+=sin(l)+2.4*p;r+=(0.5+0.5*cos(l))/sc;sc*=1.2;
        }
        return r.x+r.y;
      }
      void main(){
        vec2 u=.5*vUv;u.x*=uR;
        vec2 p=vUv-uP;p.x*=uR;
        float p2=clamp(length(p),0.,1.);p2=.5*pow(1.-p2,2.);
        float n=ns(u,0.0008*uT,p2);
    ...
    {CORNERS.map(({ cls, d }, i) => (
      <span key={i} className={`absolute ${cls} w-[10px] h-[10px] pointer-events-none`}>
        <svg width="10" height="10" viewBox="0 0 10 10" fill="none">
          <path
            d={d}
            stroke={hovered ? "#dc2626" : "rgba(220,38,38,0.28)"}
            strokeWidth="2"
...
const CardBorder = ({ hovered }: { hovered: boolean }) => (
  <>
    <motion.div
      ...
      animate={{
        boxShadow: hovered
          ? "0 0 0 1px #dc2626, 0 8px 32px rgba(220,38,38,0.22), 0 0 60px rgba(220,38,38,0.08)"
    ...
    <AnimatePresence>
      {hovered && (
        <motion.div
          aria-hidden
          className="pointer-events-none absolute inset-x-0 z-30 h-px"
          style={{
    ...
    <motion.div
      ...
      style={{
        background: "linear-gradient(to right, #dc2626, #ef4444, transparent)",
      }}
      ...
      >
        ...
        <div className="relative w-full overflow-hidden" style={{ aspectRatio: "16 / 9" }}>
          ...
          <div
            ...
            style={{
              background:
                "linear-gradient(to top, rgba(6,6,6,.92) 0%, rgba(6,6,6,.25) 55%, transparent 100%)",
            }}
          />
          {/* REC badge */}
          {service.isVlog && (
            <div className="absolute top-2 right-2 z-10 flex items-center gap-1 bg-black/80 px-[7px] py-[3px] border border-white/10">
              <span
                ...
                style={{ animation: "adBlink 1.4s ease-in-out infinite" }}
              />
              <span className="text-[8px] font-bold text-white tracking-[.12em]">REC</span>
            </div>
          )}
          {/* Coming soon badge — top-left, fully clipped inside card */}
          {service.comingSoon && (
            <div
            ...
            >
              <span
                className="w-[4px] h-[4px] rounded-full bg-white flex-shrink-0"
                style={{ animation: "adBlink 1.4s ease-in-out infinite" }}
              />
              <span className="text-white font-black text-[7px] tracking-[.15em] uppercase">
                COMING SOON
              </span>
            </div>
          )}
        ...
        <div className="px-3 pt-[10px] pb-1">
          <motion.p
            className="text-red-600 font-black text-[8px] tracking-[.3em] mb-[5px]"
          ...
          <motion.h3
            className="font-black italic uppercase leading-tight text-white mb-[5px]"
            style={{ fontFamily: "'Orbitron', sans-serif", fontSize: "11.5px" }}
            animate={{ x: hovered ? 2 : 0 }}
            transition={{ duration: 0.2 }}
          >
            {service.titleWhite}{" "}
          ...
          <p className="text-[9.5px] text-zinc-500 uppercase tracking-[.1em] leading-[1.5]">
            {service.description}
          </p>
        </div>
        ...
        <div className="px-3 pb-[10px] pt-[6px] flex justify-end">
          {!service.comingSoon && (
            <motion.div
              ...
              animate={{
                borderColor: hovered ? "#dc2626" : "rgba(220,38,38,0.3)",
                background:  hovered ? "rgba(220,38,38,0.14)" : "transparent",
              }}
              transition={{ duration: 0.2 }}
            >
...
const OrbitRing = ({ radius, delay = 0 }: { radius: number; delay?: number }) => (
  <div
    ...
    style={{
      ...
      animation: `adRingPulse 5s ease-in-out ${delay}s infinite`,
    }}
  />
);
  ...
  return (
    <motion.button
      ...
      animate={{
        ...
        boxShadow: hovered
          ? "0 0 0 1px rgba(220,38,38,0.4), 0 0 40px rgba(220,38,38,0.6), 0 0 80px rgba(220,38,38,0.2)"
    ...
    >
      ...
      <span
        className="relative z-10 text-white font-black italic tracking-[.22em] uppercase text-[10px]"
        style={{ fontFamily: "'Orbitron', sans-serif" }}
      >
...
export default function OurServices() {
  ...
  useEffect(() => {
    ...
    const tick = (ts: number) => {
      if (lastTs.current !== null && !paused) {
        setTime((t) => t + (ts - lastTs.current!) / 1000);
      }
      lastTs.current = ts;
  ...
  return (
    ...
    >
      ...
      {/* Radial red glow — no grid */}
      <div
        aria-hidden
        className="absolute inset-0 z-[1] pointer-events-none"
        style={{
          background:
            "radial-gradient(ellipse 60% 55% at 50% 50%, rgba(220,38,38,0.07) 0%, transparent 68%)",
        }}
      />
      ...
      <div className="relative z-10 flex flex-col items-center px-5 sm:px-8 py-14 lg:py-20 max-w-3xl mx-auto">
        ...
        >
          ...
          <span
            className="text-red-600 font-bold italic text-[9px] tracking-[.35em] uppercase"
            style={{ fontFamily: "'Orbitron', sans-serif" }}
          >
            AUTODOSE
          </span>
        ...
        <motion.h2
          className="font-black italic uppercase text-center leading-none mb-2"
          style={{
        ...
        >
          ...
          <span style={{ color: "#dc2626", textShadow: "0 0 24px rgba(220,38,38,0.45)" }}>
            SERVICES
          </span>
        </motion.h2>

        <motion.p
          className="text-zinc-500 text-[10px] tracking-[.2em] uppercase mb-10 text-center"
          initial={{ opacity: 0 }}
        ...
        >
          ...
          {/* Center hub — outer: pulse animation + border glow (no clip here) */}
          <div
            style={{
              position: "absolute",
              top: "50%", left: "50%",
          ...
          >
            {/* Inner: handles clip + bg + layout — separate from animation so border-radius clip works */}
            <div
              style={{
                position: "absolute",
                inset: 0,
            ...
            >
              {/* Logo — scale to fit with padding so it isn't cropped inside the smaller circle */}
              <img
                src={AUTO_DOSE_LOGO}
                alt="AutoDose"
                style={{
