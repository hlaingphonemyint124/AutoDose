...
export const isFacebookVideoUrl = (urlOrId?: string | null): boolean => {
...
export const getVideoSourceType = (urlOrId?: string | null): "youtube" | "facebook" | "upload" | null => {
  if (!urlOrId) return null;
  if (getYouTubeVideoId(urlOrId)) return "youtube";
  if (isFacebookVideoUrl(urlOrId)) return "facebook";
  return null;
};
...
export const getFacebookEmbedUrl = (
  videoUrl?: string | null,
  options: { autoplay?: boolean; muted?: boolean; controls?: boolean } = {}
): string | null => {
  if (!videoUrl || !isFacebookVideoUrl(videoUrl)) return null;
  ...
  if (options.muted !== false) params.set("mute", "true");
...
export const getVideoEmbedUrl = (
  ...
  const source = video.youtube_video_id || video.youtube_url || video.storage_url || "";

  if (video.source_type === "facebook" || isFacebookVideoUrl(video.storage_url)) {
    return getFacebookEmbedUrl(video.storage_url, options);
  }

  if (video.source_type === "youtube" || getYouTubeVideoId(source)) {
    return getYouTubeEmbedUrl(source, options);
  }

  return null;
};

export const isEmbeddedVideo = (video: {
  source_type?: string | null;
  youtube_url?: string | null;
  youtube_video_id?: string | null;
  storage_url?: string | null;
